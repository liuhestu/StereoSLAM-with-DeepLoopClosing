...
bool System::RunStep(const cv::Mat &leftImg, const cv::Mat &rightImg, const double &dTimestamp){
   ...
   if(success){
       Frame::Ptr current_frame = _mpFrontend->GetCurrentFrame();
       if(current_frame){
           std::unique_lock<std::mutex> lck(_mmutexAllFrames);
           _mvAllFrames.push_back(current_frame);
       }
   }
...
void System::SaveKeyTrajectory(std::string &save_file){
    ...
    for (auto &kf: poses_map){
        ...
        outfile << std::setprecision(6) << timestamp << " "
            ...
            << pose_q.x() << " " << pose_q.y() << " " << pose_q.z() << " " << pose_q.w() << std::endl;
    }
    outfile.close();
}
...
// the output format is like:
//      "timestamp, tx, ty, tz, qx, qy, qz, qw" per line
void System::SaveFrameTrajectory(std::string &save_file){
    std::ofstream outfile;
    outfile.open(save_file, std::ios_base::out|std::ios_base::trunc);
    outfile << std::fixed;

    std::vector<Frame::Ptr> frame_list;
    {
        std::unique_lock<std::mutex> lck(_mmutexAllFrames);
        frame_list = _mvAllFrames;
    }

    std::sort(frame_list.begin(), frame_list.end(), [](const Frame::Ptr &left, const Frame::Ptr &right){
        return left->mnFrameId < right->mnFrameId;
    });

    for(const auto &frame : frame_list){
        double timestamp = frame->mdTimeStamp;
        SE3 frame_pose = frame->Pose().inverse();
        Vec3 pose_t = frame_pose.translation();
        Mat33 pose_R = frame_pose.rotationMatrix();
        Eigen::Quaterniond pose_q = Eigen::Quaterniond(pose_R);
    ...
    for (auto &kf: poses_map){
        ...
        if(loopKF){
            ...
            outfile << std::setprecision(6) << keyframe_id << " " << timestamp_current << " "
                << pose_t_current.x() << " " << pose_t_current.y() << " " << pose_t_current.z() << " "
                << pose_q_current.x() << " " << pose_q_current.y() << " " << pose_q_current.z() << " " << pose_q_current.w() << std::endl;
            ...
            outfile << std::setprecision(6) << loopKF->mnKFId << " " << timestamp_loop << " "
                << pose_t_loop.x() << " " << pose_t_loop.y() << " " << pose_t_loop.z() << " "
                << pose_q_loop.x() << " " << pose_q_loop.y() << " " << pose_q_loop.z() << " " << pose_q_loop.w() << std::endl;
        }
    }
    outfile.close();
}
